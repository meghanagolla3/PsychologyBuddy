...
async function main() {
  ...
  const counselorRole = await upsertRole("COUNSELOR", "School counselor with access to students requiring mental health support");

  const parentRole = await upsertRole("PARENT", "Parent or guardian of a student user");
  ...
  for (const key of ROLE_PERMISSIONS.SCHOOL_SUPERADMIN) {

    const perm = permissionRecords.find((p: { name: string; id: string }) => p.name === key);

    if (perm) await upsertRolePermission(schoolSuperAdminRole.id, perm.id);

  }



  for (const key of ROLE_PERMISSIONS.COUNSELOR) {

    const perm = permissionRecords.find((p: { name: string; id: string }) => p.name === key);

    if (perm) await upsertRolePermission(counselorRole.id, perm.id);

  }



  for (const key of ROLE_PERMISSIONS.PARENT) {

    const perm = permissionRecords.find((p: { name: string; id: string }) => p.name === key);

    if (perm) await upsertRolePermission(parentRole.id, perm.id);
  ...
  // 8. Create Counselor User

  // ------------------------------------------------

  const counselorUser = await upsertUser(

    "counselor@calmpath.ai",
  ...
  // Update counselor user with phone number for OTP testing
  ...
  // Create AdminProfile for Counselor
  ...
  // 10. Create Default Music Categories
