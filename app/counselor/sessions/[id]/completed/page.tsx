    ...
    try {
      ...
      if (sessionData.sessionType === 'FOLLOW_UP') {
        ...
        while (currentPreviousSessionId && reportTraversalCount < maxReportTraversal) {
          try {
            console.log(`  - Checking session: ${currentPreviousSessionId} (iteration ${reportTraversalCount + 1})`);
            ...
            if (!previousSessionResult.success) {
              console.log(`  - Failed to get session data for ${currentPreviousSessionId}:`, previousSessionResult.message);
              break;
            }
            
            const sessionData = previousSessionResult.data;
            console.log(`  - Session ${currentPreviousSessionId} type: ${sessionData?.sessionType}`);
            console.log(`  - Session ${currentPreviousSessionId} previousSessionId: ${sessionData?.previousSessionId}`);
            
            if (sessionData?.sessionType === 'FOLLOW_UP') {
              // Fetch report if it's a follow-up session
              console.log(`  - Fetching report for follow-up session: ${currentPreviousSessionId}`);
              ...
              if (previousReportResult.success) {
                ...
                console.log(`  - ✅ Found follow-up report for session: ${currentPreviousSessionId}`);
              } else {
                console.log(`  - ❌ Failed to get report for ${currentPreviousSessionId}:`, previousReportResult.message);
              }
            } else {
              console.log(`  - ⏹️ Session ${currentPreviousSessionId} is not a follow-up (${sessionData?.sessionType}), stopping report collection`);
            ...
            console.log(`  - Next session to check: ${currentPreviousSessionId || 'null'}`);
          } catch (err) {
            console.log(`  - ❌ Failed to fetch report for session ${currentPreviousSessionId}:`, err);
            break;
          }
        ...
        console.log(`  - Total previous follow-up reports found: ${allPreviousReports.length}`);
        ...
        console.log('🔍 DEBUG: Session Reports Collection:');
        console.log('  - All previous follow-up reports:', allPreviousReports.length);
        console.log('  - Current session ID:', sessionId);
        console.log('  - Current report data:', currentReportResult.data);
        ...
        if (sessionData.previousSessionId && originalIntakeData) {
          console.log('  - Creating SESSION REPORT 1 from original intake session data');
          console.log('  - Original intake data:', originalIntakeData);
          console.log('  - Original intake sessionReport:', originalIntakeData?.sessionReport);
          ...
          console.log('  - Found intake behavioral tags:', intakeBehavioralTags);
          ...
          console.log('  - Intake report created with behavioralTags:', intakeReportFromOriginal.behavioralTags);
          console.log('  - Full intake report object:', intakeReportFromOriginal);
          sessionReports.push({
            sessionNumber: 1,
        ...
        allPreviousReports.forEach((prevReport, index) => {
          ...
          console.log(`  - Adding previous follow-up session report as SESSION REPORT ${reportNumber}`);
          sessionReports.push({
        ...
        console.log(`  - Adding current follow-up session report as SESSION REPORT ${currentSessionNumber}`);
        sessionReports.push({
        ...
        console.log(`  - Final session reports array (${sessionReports.length} reports):`, sessionReports);
    ...
    <div className="min-h-screen bg-[#F8FAFC] p-4 md:p-5">
      <div className="max-w-7xl mx-auto">
        ...
        <div id="intake-preview" className="rounded-[16px] border border-[#E2E8F0] bg-[#FFFFFF] p-10 shadow-sm">
          ...
          {isFollowUpSession ? (
            <>
              {/* Follow-up Session Preview - Exact Match from Follow-up Page */}
              ...
              {/* Include all intake sections like in follow-up page */}
              {intakeData.factors && (
                <>
              ...
              {(() => {
                console.log('🔍 DEBUG: Rendering Session Reports:');
                console.log('  - intakeData.sessionReports:', intakeData.sessionReports);
                console.log('  - Length:', intakeData.sessionReports?.length || 0);
                return intakeData.sessionReports && intakeData.sessionReports.length > 0;
              })() ? (
                intakeData.sessionReports?.map((sessionReportData, index) => {
                  console.log(`🔍 DEBUG: Rendering SESSION REPORT ${sessionReportData.sessionNumber}:`);
                  console.log(`  - Session ID: ${sessionReportData.sessionId}`);
                  console.log(`  - Behavioral Tags:`, sessionReportData.report.behavioralTags);
                  console.log(`  - Tags Length: ${sessionReportData.report.behavioralTags?.length || 0}`);
                  ...
                  <div key={sessionReportData.sessionId}>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {(() => {
                        console.log(`🎨 RENDERING TAGS for SESSION REPORT ${sessionReportData.sessionNumber}:`);
                        console.log(`  - Tags exist: ${!!sessionReportData.report.behavioralTags}`);
                        console.log(`  - Tags array:`, sessionReportData.report.behavioralTags);
                        console.log(`  - Tags length: ${sessionReportData.report.behavioralTags?.length || 0}`);
                        
                        if (sessionReportData.report.behavioralTags && sessionReportData.report.behavioralTags.length > 0) {
                          console.log(`  - Rendering ${sessionReportData.report.behavioralTags.length} tags`);
                          return sessionReportData.report.behavioralTags.map((behavior, tagIndex) => {
                            console.log(`    - Rendering tag ${tagIndex}: "${behavior}"`);
                            return (
                              <span
                          ...
                          console.log(`  - Showing "Not specified"`);
                          return (
                ...
                <>
                  {/* Fallback to single session report for backward compatibility */}
