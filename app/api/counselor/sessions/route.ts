...
import { CounselingService, createSessionSchema, updateSessionSchema, startSessionSchema, completeSessionSchema, cancelSessionSchema } from '@/src/server/counseling/counseling.service';
...
export const POST = withPermission({
  ...
  try {
    ...
    if (error.message.includes('already been scheduled for this alert')) {
      return NextResponse.json(
        { success: false, message: error.message, error: error.message },
        { status: 400 }
      );
    ...
    if (error.name === 'ZodError') {
      return NextResponse.json(
        { success: false, message: 'Invalid input data', errors: error.errors, error: error.message },
        { status: 400 }
      );
    }
