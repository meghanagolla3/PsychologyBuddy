...
export async function GET(request: NextRequest) {
  try {
    ...
    return NextResponse.json({
      success: true,
      data: challenges,
      message: "Music challenges retrieved successfully"
    });
    ...
    console.error('Error fetching music challenges:', error);
    return NextResponse.json(
      {
        error: 'Failed to fetch music challenges',
...
export async function POST(request: NextRequest) {
  try {
    ...
    return NextResponse.json({
      success: true,
      data: userChallenge,
      message: "Music challenge started successfully"
    });
    ...
    console.error('Error starting music challenge:', error);
    return NextResponse.json(
      {
        error: 'Failed to start music challenge',
        details: error instanceof Error ? error.message : 'Unknown error',
      },
      { status: 500 }
    );
