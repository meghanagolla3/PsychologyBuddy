...
export async function GET(request: NextRequest) {
  try {
    ...
    return NextResponse.json({
      success: true,
      data: challenges,
      message: "Meditation challenges retrieved successfully"
    });
    ...
    console.error('Error fetching meditation challenges:', error);
    return NextResponse.json(
      {
        error: 'Failed to fetch meditation challenges',
...
export async function POST(request: NextRequest) {
  try {
    ...
    return NextResponse.json({
      success: true,
      data: userChallenge,
      message: "Meditation challenge started successfully"
    });
    ...
    console.error('Error starting meditation challenge:', error);
    return NextResponse.json(
      {
        error: 'Failed to start meditation challenge',
        details: error instanceof Error ? error.message : 'Unknown error',
      },
      { status: 500 }
    );
