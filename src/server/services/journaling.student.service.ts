import { JournalingStudentRepository } from '../repository/journaling.student.repository';
...
export class JournalingStudentService {
  // Get journaling configuration for a school
  static async getJournalingConfig(schoolId: string) {
    try {
      ...
      return {
        success: true,
        message: 'Journaling configuration retrieved successfully',
        data: config,
      };
    } catch (error) {
      console.error('Get journaling config error:', error);
      throw new AuthError('Failed to retrieve journaling configuration', 500);
    }
  }
