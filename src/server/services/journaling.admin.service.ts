import { JournalingAdminRepository } from '../repository/journaling.admin.repository';
...
export class JournalingAdminService {
  ...
  static async getJournalingConfig(userId: string, query: GetJournalingConfigInput) {
    try {
      ...
      if ((admin.role.name !== 'SUPER_ADMIN' && admin.role.name !== 'SUPERADMIN') && admin.schoolId !== query.schoolId) {
  ...
  static async updateJournalingConfig(userId: string, data: UpdateJournalingConfigInput) {
    try {
      ...
      // For super admins, use the schoolId from the payload if provided
      if ((admin.role.name === 'SUPER_ADMIN' || admin.role.name === 'SUPERADMIN') && data.schoolId) {
        schoolId = data.schoolId;
      }
      
      if (!schoolId) {
        throw new AuthError('School ID is required', 400);
      }

      // Remove schoolId from data before updating config (it's not a config field)
      const { schoolId: _, ...configData } = data;

      const config = await JournalingAdminRepository.updateJournalingConfig(schoolId, configData);
