...
export class CounselingRepository {
  ...
  async createSession(data: CreateSessionData) {
    ...
    // Check if a session already exists for this escalation alert
    if (data.escalationId) {
      const existingSession = await prisma.counselingSession.findUnique({
        where: { escalationId: data.escalationId }
      });
      if (existingSession) {
        throw new Error('A session has already been scheduled for this alert');
      }
    }
    ...
    try {
      ...
      console.error('[CounselingRepository] Error in createSession:', error);
      throw error;
    }
  ...
  async getSessions(
    ...
    // Only add schoolId filter if it's provided (SUPERADMINs can see all schools)
    if (schoolId) {
